...
#if !(defined(__linux) || defined(__linux__))
			cout << "    老师的机器（CPU：AMD Ryzen 7 Pro 4750U, 内存：DDR4 3200MHz）运行VS2022-Debug-x86编译的程序，大约耗时250-270秒" << endl;
#else
			cout << "    Linux服务器下运行，大约耗时125-145秒（如果多人同时测试，偏差可能较大）" << endl;
#endif
		}
		else {
			cout << "内存性能测试(s1+=str方式)" << endl;
#if !(defined(__linux) || defined(__linux__))
			cout << "    老师的机器（CPU：AMD Ryzen 7 Pro 4750U, 内存：DDR4 3200MHz）运行VS2022-Debug-x86编译的程序，大约耗时110-130秒" << endl;
#else
