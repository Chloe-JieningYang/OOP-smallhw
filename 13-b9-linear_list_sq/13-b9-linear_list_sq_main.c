...
int main()
{
    ...
    cur_e = (ElemType)malloc(80 * sizeof(char));//cur_e实际是指针，必须先申请相应的空间
    for (i=INSERT_NUM*2; i> 0; i-=2) {
	sprintf(e1, "test%03d", i);
	ListInsert(&L, 1, e1);
	}
    ...
    else
    {
        ...
        {
#ifdef ELEMTYPE_IS_DOUBLE
            printf("在%5.1f前插入元素=%5.1f成功\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
            printf("在%s前插入元素=%s成功\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
            printf("在\"%d\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e.num, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
            printf("在\"%d\"前插入元素=\"%d.%s.%c.%f.%s\"成功\n", cur_e->num, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
            printf("在%d前插入元素=%d成功\n", cur_e, e1);
...
#ifdef ELEMTYPE_IS_DOUBLE
            printf("在%5.1f前插入元素=%5.1f失败\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_CHAR_ARRAY) || defined (ELEMTYPE_IS_CHAR_P)
            printf("在%s前插入元素=%s失败\n", cur_e, e1);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT)
            printf("在\"%d\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", cur_e.num, e1.num, e1.name, e1.sex, e1.score, e1.addr);
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
            printf("在\"%d\"前插入元素=\"%d.%s.%c.%f.%s\"失败\n", cur_e->num, e1->num, e1->name, e1->sex, e1->score, e1->addr);
#else	//缺省当做int处理
            printf("在%d前插入元素=%d失败\n", cur_e, e1);
    ...
    printf("要测试按位序删除or按指定值删除(多个只删第一个)or按指定值删除(多个全部删除)?(对应选项1/2/3)");
    scanf("%d", &choice);
    if (choice == 1)
    {
        printf("\n请输入要删除元素的位序：\n");
