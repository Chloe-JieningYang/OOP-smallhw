...
//		cout << "s1应为HelloHello!HelloHello!HelloHello!，实际输出：" << (((s1 *= 2) + '!') *= 3) << endl;
	...
	if (1)
	{
		...
		cout << "尾部追加(append)简单测试(TStringAdv类和TStringAdv类、字符指针、字符数组、字符)" << endl;
	...
	if (1)
	{
		...
		cout << "插入(insert)正确测试(插入TStringAdv、字符指针、字符数组、字符)" << endl;

		cout << "s1应为row the boat，                     实际输出：" << s1 << endl;

		s1.insert(s2, 4);
		cout << "s1应为row row the boat，                 实际输出：" << s1 << endl;

		s1.insert(str3, 8);
		cout << "s1应为row row row the boat，             实际输出：" << s1 << endl;

		s1.insert(str4, 1);
		cout << "s1应为Let's sing: row row row the boat， 实际输出：" << s1 << endl;
	...
	if (1)
	{
		...
		cout << "插入(insert)非法测试(非法插入TStringAdv、字符指针、字符数组、字符)" << endl;

		cout << "s1应为row the boat，实际输出：" << s1 << endl;

		s1.insert(s2, -1);//pos越界
		cout << "s1应为row the boat，实际输出：" << s1 << endl;

		s1.insert(s2, 14);//pos越界
		cout << "s1应为row the boat，实际输出：" << s1 << endl;
		...
		s1.insert(str4, 0);//pos越界
		cout << "s1应为row the boat，实际输出：" << s1 << endl;

		s1.insert(str5, 100);//pos越界
		cout << "s1应为row the boat，实际输出：" << s1 << endl;
	...
	if (1)
	{
		...
		cout << "删除(erase)测试(TStringAdv、字符指针、字符串、字符)" << endl;
		
		cout << "s1应为rowtheboat，       实际输出：" << s1 << endl;

		s1.erase(s2);
		cout << "s1应为theboat，          实际输出：" << s1 << endl;

		s1.insert(s2, 8);
		s1.insert(s2, 1);
		s1.erase("row");
		cout << "s1应为theboatrow，       实际输出：" << s1 << endl;
		...
		for (int k = 1; k <= 2; k++)
			s1.erase('?');
		cout << "s1应为whywhywhy?the，    实际输出：" << s1 << endl;
	...
	if (1)
	{
		...
		cout << "测试取子串(substr)" << endl;

		s3 = s1.substr(6);//缺省取到末尾
		cout << "s3应为hundred-twenty five thousand six hundred minutes,      实际输出：" << s3 << endl;

		s3 = s1.substr(1, 4);
		cout << "s3应为Five，                                                 实际输出：" << s3 << endl;

		s3 = s1.substr(6, 14);
		cout << "s3应为hundred-twenty，                                       实际输出：" << s3 << endl;

		s3 = s1.substr(60);//pos超出范围
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;

		s3 = s1.substr(100, 2);//pos超出范围
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;

		s3 = s1.substr(1, 0);//len为0
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;

		s3 = s1.substr(1, -1);//len为负数
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;

		s3 = s1.substr(1, 100);//len超出范围
		cout << "s3应为Five hundred-twenty five thousand six hundred minutes，实际输出：" << s3 << endl;

		s3 = s2.substr(1);//取空串的子串
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;

		s3 = s2.substr(1, 1);//取空串的子串
		cout << "s3应为<NULL>，                                               实际输出：" << s3 << endl;
	...
	if (1)
	{
		...
		cout << "s1是" << s1 << "，依次打印各个字符为：";
		for (int i = 0; i < s1.length(); i++)
			cout << s1.at(i) << ' ';
		cout << endl << endl;

		cout << "s1是" << s1 << "，依次打印字符int值的结果是（有越界）：" << endl;
		for (int i = -4; i < s1.length() + 4; i++)
			cout << int(s1.at(i)) << ' ';
		cout << endl << endl;

		s1.at(0) = s1.at(0) - 32;
		cout << "s1是" << s1 << "，依次打印字符的结果是：";
		for (int i = 0; i < s1.length(); i++)
			cout << s1.at(i) << ' ';
		cout << endl << endl;

		s1.at(2) = 'u';
		cout << "s1应为House，实际输出：" << s1 << endl;
