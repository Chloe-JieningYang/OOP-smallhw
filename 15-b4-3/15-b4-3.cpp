...
int read()
{
    ...
    for (int n = 0; n <= 15; n++)
    {
        gfile.read((char*)&player.name[n], sizeof(char));
    }
    gfile.read((char*)&player.life, sizeof(player) - sizeof(player.name));
    */

    char* c = (char*)(&player);
    for (int n = 0; n < sizeof(player); n++)
        *(c + n) = gfile.get();
    ...
    */
    ...
    while (1)
    {
        ...
        else if (choice == '1')
        {
            ...
            for (int n = 0; n <= 15; n++)
            {
                gfile.write((char*)&player.name[n], sizeof(char));
            }
            gfile.write((char*)&player.life, sizeof(player) - sizeof(player.name));
            */
            unsigned char* c = (unsigned char*)(&player);
            for (int n = 0; n < sizeof(player); n++)
            {
                gfile.put(c[n]);
            }
